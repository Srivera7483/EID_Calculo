...
# Tipo según d8 mod 3:  0 → removible | 1 → salto | 2 → infinita
...
def calcularLimitesLaterales(formula, puntoAnalisis, tipo, digitos):
    """Calcula los límites laterales aproximándose al punto desde ambos lados."""
    pasos = []
    d1, d2, d3, d4, d5, d6, d7, d8 = digitos
    ...
    def evaluar(x):
        """Evalúa f(x). Retorna None si no está definida en ese punto."""
        try:
            if tipo == 'removible':
                if x == d3:
                    return None
    ...
    # Acercarse al punto desde la izquierda y desde la derecha
    ...
    for i in range(4):
        xIzq = puntoAnalisis + desplazamientosIzquierda[3 - i]
        fIzq = valoresIzquierda[xIzq]
        textofIzq = f"{fIzq:.4f}" if fIzq is not None else "Indefinido"

        xDer = puntoAnalisis + desplazamientosDerecha[i]
    ...
    # El límite se estima con el valor más cercano al punto (x = a ± 0.001)
...
def analizarContinuidad(limites, puntoAnalisis, tipo):
    ...
    esContinua = False  # Ninguno de los tres tipos es continuo en el punto
